...
# Set the composite node attributes here.
...
when "debian"
  ...
  case
  when node['platform_version'].to_f < 6.0 # All 5.X
    node.default['postgresql']['server']['service_name'] =
  ...
  node.default['postgresql']['client']['packages'] =
    ["postgresql-client-#{node['postgresql']['version']}","libpq-dev"]
...
when "redhat", "centos", "scientific", "oracle"
  ...
  if node['platform_version'].to_f >= 6.0 && node['postgresql']['version'] == '8.4'
    ...
    node.default['postgresql']['contrib']['packages'] =
      ["postgresql#{node['postgresql']['version'].split('.').join}-contrib"]
  end
...
when 'debian'
  ...
  node.default['postgresql']['config']['hba_file'] =
    "/etc/postgresql/#{node['postgresql']['version']}/main/pg_hba.conf"
  node.default['postgresql']['config']['ident_file'] =
    "/etc/postgresql/#{node['postgresql']['version']}/main/pg_ident.conf"
  node.default['postgresql']['config']['external_pid_file'] =
  ...
  node.default['postgresql']['config']['unix_socket_directory'] =
    '/var/run/postgresql' if node['postgresql']['version'].to_f < 9.3
  node.default['postgresql']['config']['unix_socket_directories'] =
    '/var/run/postgresql' if node['postgresql']['version'].to_f >= 9.3
  node.default['postgresql']['config']['shared_buffers'] = '24MB'
  node.default['postgresql']['config']['max_fsm_pages'] =
    153600 if node['postgresql']['version'].to_f < 8.4
